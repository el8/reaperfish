...
// misnamed, not a histogram but just the average of IO times
struct hist {
        __u64 read_time;
	__u64 read_nr;
        __u64 write_time;
...
int BPF_PROG(trace_bio_done, struct request_queue *q, struct bio *bio)
{
	...
	if (rwflag == REQ_OP_READ) {
		__sync_fetch_and_add(&histp->read_nr, 1);
		__sync_fetch_and_add(&histp->read_time, delta);
	}
	else if (rwflag == REQ_OP_WRITE) {
		__sync_fetch_and_add(&histp->write_nr, 1);
		__sync_fetch_and_add(&histp->write_time, delta);
	}
